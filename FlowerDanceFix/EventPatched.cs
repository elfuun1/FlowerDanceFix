        ...
        public static void setUpFestivalMainEvent_FDF(StardewValley.Event __instance)
        {
            ...
            try
            {
                ...
                */
                ...
                List<string> usesFDFCustomSprites = new List<string>();
                        ...
                        switch (intgender)
                        {
                            ...
                            case 2:
                                if (Config.AllowNonBinaryPartners.Equals(true))
                                {
                                    try
                                    {
                                        string assetPath = "assets/" + character.Name + "_FDF";

                                        Texture2D texture = Helper.Content.Load<Texture2D>(assetPath, ContentSource.ModFolder);
                                        ...
                                        usesFDFCustomSprites.Add(character.Name);

                                        break;
                                    }
                                    catch (Exception)
                                    {
                                        Monitor.Log("Failed to added " + character.Name + " to a leftoverGender dancer pool- assets/" + character.Name + "_FDF.png does not exist or could not be read", LogLevel.Debug);
                                        break;
                                    }
                                }
                                else
...

                {
                    ...
                    /*
                    //Sets up static event command strings
                    string setupFestivalData = "pause 500/playMusic none/pause 500/globalFade/viewport -1000 -1000/loadActors MainEvent";
                    string startEvent = "	/viewport 14 25 clamp true/ pause 2000 / playMusic FlowerDance/ pause 600";
                    string endEvent = "	/pause 3000/ globalFade / viewport - 1000 - 1000/ message \"That was fun! Time to go home...\"/ waitForOtherPlayers festivalEnd / end";

                    //Warp in Farmers
                    StringBuilder warpInFamers = new StringBuilder();

                    //Sets up variable NPC event command strings
                    int x = 11;
                    int j = 0;
                    int k = -2;
                    int limit = 1;

                    StringBuilder spawnDancers = new StringBuilder();
                    StringBuilder setFrame = new StringBuilder();
                    StringBuilder animate1 = new StringBuilder();
                    StringBuilder animate2 = new StringBuilder();
                    StringBuilder positionOffsetBlock = new StringBuilder();
                    StringBuilder animate3 = new StringBuilder();
                    StringBuilder stopAnimate = new StringBuilder();

                    while (limit < females.Count())
                    {
                        spawnDancers.Append("/warp Girl" + limit + " " + (x + j) + " " + "24");
                        spawnDancers.Append("/warp Guy" + limit + " " + (x + j) + " " + "27");

                        setFrame.Append("/showFrame Girl" + limit + " " + "40");
                        setFrame.Append("/showFrame Guy" + limit + " " + "44");

                        animate1.Append("/animate Girl" + limit + "false true 600 43 41 43 42");
                        animate1.Append("/animate Guy" + limit + "false true 600 44 45");

                        animate2.Append("/animate Girl" + limit + " false true 600 46 47");
                        animate2.Append("/animate Guy" + limit + " false true 300 46 47");

                        positionOffsetBlock.Append("/positionOffset Guy" + limit + " 0 -2");

                        animate3.Append("/animate Girl" + limit +" false true 600 43 41 43 42");
                        animate3.Append("/animate Guy" + limit + " false true 600 44 45");

                        stopAnimate.Append("/stopAnimation Girl" + limit);
                        stopAnimate.Append("/stopAnimation Guy" + limit);
                        limit++;
                        j += 2;

                        spawnDancers.Append("/warp Girl" + limit + " " + (x + k) + " " + 24);
                        spawnDancers.Append("/warp Guy" + limit + " " + (x + k) + " " + 27);
                    ...
                    //finish setting up positionOffset blocks
                    positionOffsetBlock.Append("/pause 300");
                    StringBuilder positionOffset = new StringBuilder();
                    int offsetLoop = 1;

                    while (offsetLoop < 29)
                    {
                        positionOffset.Append(positionOffsetBlock);
                        offsetLoop++;
                    }

                    //create newFestival Data
                    StringBuilder newFestivalData = new StringBuilder();
                    newFestivalData.Append(setupFestivalData);
                    newFestivalData.Append(warpInFamers);
                    newFestivalData.Append(spawnDancers);
                    newFestivalData.Append(setFrame);
                    newFestivalData.Append(startEvent);
                    newFestivalData.Append(animate1);
                    newFestivalData.Append("/ pause 9600");
                    newFestivalData.Append(positionOffset);
                    newFestivalData.Append(animate3);
                    newFestivalData.Append("/pause 7600");
                    newFestivalData.Append(stopAnimate);
                    newFestivalData.Append(endEvent);
                    Monitor.Log("Successfully created new mainEvent data with " + females.Count + "pairs", LogLevel.Debug);
                    */
                    int i = 1;
                    do
                    {
